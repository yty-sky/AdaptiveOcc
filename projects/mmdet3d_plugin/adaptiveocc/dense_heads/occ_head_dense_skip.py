...
from projects.mmdet3d_plugin.adaptiveocc.loss.loss_utils import multiscale_supervision, geo_scal_loss, sem_scal_loss
...
class OccHead_DenseSkip(nn.Module):
    def __init__(self,
        ...
        self.is_gn = is_gn
        self._init_layers()
        self.octree = Octree(depth=depth, pc_range=[-50, -50, -5.0, 50, 50, 3.0], occ_size=[volume_h, volume_w, volume_z])
    ...
    def forward(self, mlvl_feats, img_metas, gt_occ=None):

        bs, num_cam, _, _, _ = mlvl_feats[0].shape
        dtype = mlvl_feats[0].dtype

        volume_queries = self.volume_embedding.weight.to(dtype)   # [10000, 512]
        self.octree = self.octree.cuda() # 操作一下
        ...
        for i in range(self.fpn_level):
            ...
            if self.is_train == True:
                ...
                split = (torch.argmax(occ_pred, dim=-1) == 17)
                self.octree.octree_split(split, i)  # 有一个特性，当全部为False时，分裂第一个
                self.octree.octree_grow_down(i + 1)
                volume_queries = self.upsample[i](volume_embed_i, self.octree, i)
                volume_queries_list.append(volume_queries)
