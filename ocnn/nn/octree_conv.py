...
class OctreeConvBase:
  ...
  def __init__(self, in_channels: int, out_channels: int,
    ...
    self.kernel_size = resize_with_last_val(kernel_size) # [3]->[3,3,3]
  ...
  def setup(self, octree: Octree, depth: int):
    ...
    if self.stride == 2:
      if self.is_conv_layer():
        ...
        self.out_depth = depth + 1     # 准备输出的深度
    ...
    if self.nempty:
      ...
      self.in_h = octree.nnum[self.in_depth]    # 最后一层的输入体素数
      self.out_h = octree.nnum[self.out_depth]  # 最后一层的输出体素数
      if self.stride == 2:
        if self.is_conv_layer():
          self.out_h = octree.nnum_nempty[self.out_depth]
        else:
  ...
  def forward_gemm(self, out: torch.Tensor, data: torch.Tensor,
    ...
    for i in range(self.buffer_n):
      ...
      # Perform octree2col
      neigh_i = self.neigh[start:end]  # [N, 27] : [6648, 27]
      valid = neigh_i >= 0
      buffer.fill_(0)
      buffer[valid] = data[neigh_i[valid]]   # 将存在得邻域对应的data赋值给buffer，获取buffer相当于N个keys，每个keys保存其邻域的特征

      # The sub-matrix gemm  [3352, 27, 4] [27, 4, 16]
  ...
  def backward_gemm(self, out: torch.Tensor, grad: torch.Tensor,
    ...
    for i in range(self.buffer_n):
      ...
      # The sub-matrix gemm
      buffer = torch.mm(grad[start:end], weights.flatten(0, 1).t())  # 可以详细看一下矩阵求导的部分
...
class OctreeConvFunction(Function):
  ...
  def forward(ctx, data: torch.Tensor, weights: torch.Tensor, octree: Octree,
    ...
    out = octree_conv.check_and_init(data)  # 具有输出形状的全0向量
...
class OctreeConv(OctreeConvBase, torch.nn.Module):
  ...
  def __init__(self, in_channels: int, out_channels: int,
    ...
    self.weights = torch.nn.Parameter(torch.Tensor(*self.weights_shape))  # 创建学习参数
    if self.use_bias:
      self.bias = torch.nn.Parameter(torch.Tensor(out_channels))
    self.reset_parameters()  # 初始化权重，没研究过初始化权重的影响
...
class OctreeDeconv(OctreeConv):
  ...
  def forward(self, data: torch.Tensor, octree: Octree, depth: int):
    ...
    if self.stride == 2:
      depth_col = depth + 1
      if not self.nempty:
        data = octree_depad(data, octree, depth)   # 把非空元素挑出来
