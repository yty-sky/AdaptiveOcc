...
class Transform:
  ...
  def __call__(self, sample: dict, idx: int):
    r''''''
    # sample: 点和法向量
    points = self.preprocess(sample, idx)  # 生成Point对象
    output = self.transform(points, idx)  #法向量取绝对值、点云范围裁剪: Points对象被更改了，但保留了原始的mask
    output['octree'] = self.points2octree(output['points']) # 点云转octree
    return output
  ...
  def preprocess(self, sample: dict, idx: int):
    ...
    xyz = torch.from_numpy(sample['points']) # numpy->tensor
    normals = torch.from_numpy(sample['normals'])
    points = Points(xyz, normals)  #生成point对象
    return points
  ...
  def transform(self, points: Points, idx: int):
    ...
    if self.orient_normal:
      points.orient_normal(self.orient_normal)  # 法向量取了绝对值
...
class CollateBatch:
  ...
  '''
  # 总结： keys： 当前层的keys来自于上一层非空体素的八划分
  #       children: 表示当前层的keys是否非空，是当前层的第几个非空体素
  #       neighs: 当前层的keys的邻域（这个邻域包含于当前层keys，无当前层keys外的数），且表示形式是当前层第几个key
  #       nnum: 当前层的keys数目
  #       nnum_nempty: 当前层的非空keys数目
