...
from ocnn.modules import Conv1x1BnRelu, OctreeConvBnRelu, Conv1x1Bn, OctreeConvBn, Conv1x1Gn, Conv1x1GnRelu, OctreeConvGnRelu
...
class OctreeResBlock3(torch.nn.Module):
  r''' Octree-based ResNet block in a bottleneck style. The block is composed of
  a series of :obj:`Conv1x1`, :obj:`Conv3x3`, and :obj:`Conv1x1`.
  ...
  def __init__(self, in_channels: int, out_channels: int, stride: int = 1,
               bottleneck: int = 4, nempty: bool = False,  groups: int = 8):
    super().__init__()
    self.in_channels = in_channels
    self.out_channels = out_channels
    self.bottleneck = bottleneck
    ...
    if self.stride == 2:
      self.max_pool = OctreeMaxPool(nempty)
    self.conv1x1a = Conv1x1GnRelu(in_channels, channelb, groups)
    self.conv3x3 = OctreeConvGnRelu(channelb, channelb, groups, nempty=nempty)
    self.conv1x1b = Conv1x1Gn(channelb, out_channels, groups)
    if self.in_channels != self.out_channels:
      self.conv1x1c = Conv1x1Gn(in_channels, out_channels, groups)
    self.relu = torch.nn.ReLU(inplace=True)
...
class OctreeResBlocks(torch.nn.Module):
  ...
  def __init__(self, in_channels, out_channels, resblk_num, bottleneck=4,
               nempty=False, resblk=OctreeResBlock, group=None, use_checkpoint=False):
    super().__init__()
    ...
    if group is None:
      ...
      self.resblks = torch.nn.ModuleList(
        [resblk(channels[i], channels[i + 1], 1, bottleneck, nempty, group)
         for i in range(self.resblk_num)])
